...
class Apriori:
    def __init__(self, dataset, total_txns, minSup):
    ...
    def Apriori_Algo(self):
        ...
        while(CandItemsets != list()):
            CandItemsets, Counter = self.get_K_plus_1_sets(CandItemsets)
            #print(CandItemsets, counter)
...
def main(dataset_path):
    ...
    minSup = 0.08    # enter this in terms of count not ratio or give ratio*N
    # minSup = int(input())

    # start_clock = datetime.now() # algo started
    start_clock = time() # algo started
    AprioriInst = Apriori(DATASET, N, minSup)
    ...
    print("Support Count Taken :",minSup)
